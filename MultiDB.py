...
# basic_cols=['SamplingPoint', 'lat', 'lon', 'GrossWT', 'Year', 'Month', 'region','Season']
# base_dfs = {}
# for k, v in raw_base.items():
#     combined_opt = optimize_df(v)
#     renew_cols=[c for c in combined_opt.columns if c in basic_cols or c.startswith('Total')]
#     combined_opt=combined_opt[renew_cols]
#     cal_col=[c for c in combined_opt if c not in basic_cols]
#     combined_opt=combined_opt.dropna(subset=cal_col)
#     combined_opt.info(memory_usage='deep')
#     base_dfs[k] = combined_opt
#
# with open("26compressedMinerals.pkl", "wb") as f:
#     pickle.dump(base_dfs, f)
# Speed-up: optimize data types and create indexes
...
def define_base(dfs, mineral, start_y, end_y, regions, watertypes):
    ...
    return base[mask]  # Return view, not copy
...
def select_SPtopk_fast(base, mineral, k):
    ...
    if base.empty:
        ...
        result = result.rename(columns={mcol: "monthly concentration"})
...
def build_geo_fig(point_agg, mineral, k, start_y, end_y):
    ...
    fig.update_layout(
        ...
        title=f"UK – {mineral} (Top {k}) ({start_y}-{end_y})",
...
def build_dot_fig(point_agg):
    ...
    label = list(point_agg.keys())[0]
    ma = point_agg[label].rename(columns={"monthly concentration": "Detected Concentration"})

    conc = ma["Detected Concentration"].dropna()
    if conc.empty:
        return []
    ...
    fig = px.line(
        ...
        title=f"Concentration over Time — {label}",
...
def build_table_data(point_agg):
    ...
    for m, d in point_agg.items():
        ...
        tbl = tbl.assign(MonthlyMean=tbl["average_concentration"].round(2).astype(str))
        ...
        tbl = tbl[["SamplingPoint", "WaterType", "MonthlyMean", "MonthlyStandardDivation", "NumberOfObservations"]]
...
def sync_dependents(chosen_mineral, prev_wtypes, prev_regions):
    """Update dependent dropdowns when mineral changes"""
    if chosen_mineral is None:
        chosen_mineral = DEFAULT_MINERAL

    wtypes = get_watertypes(raw_base, chosen_mineral)
    regions = ['NE', 'NW', 'SE', 'SW']

    wtypes_val = _preserve(prev_wtypes, wtypes)
    regions_val = _preserve(prev_regions, regions)

    return wtypes, wtypes_val, regions, regions_val
...
def update_all(n_click, chosen_mineral, chosen_wtypes, chosen_regions, start_y, end_y, topkn):
    ...
    # Get valid options
    all_wtypes = get_watertypes(raw_base, chosen_mineral)
    ...
    # Create a unique revision key based on parameters
    revision_key = f"{chosen_mineral}_{start_y}_{end_y}_{len(wtypes)}_{len(regions)}_{topkn}"
    ...
    wt_df = select_WTtopk(base, chosen_mineral, topkn)  # ✅ Use base, not cached version
